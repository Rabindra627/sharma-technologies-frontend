...
const INITIAL_CLIENTS = [
  ...
  },
  {
    ...
    email: "e.rostova@bchorizons.com",
    industry: "Finance",
    revenue: "$210,000",
    health: "At Risk",
  ...
  {
    id: "CLN-1044",
    name: "Stellar Health Corp",
    liaison: "David Kross",
    email: "d.kross@stellarhealth.org",
    industry: "Healthcare",
    revenue: "$34,500",
    health: "Good",
...
export default function ClientsPage() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const newClient = {
      ...
      revenue: formData.revenue.startsWith("$")
        ? formData.revenue
        : `${formData.revenue}`,
    ...
    try {
      const res = await fetch("/api/auth/clients", {
        ...
        body: JSON.stringify(newClient), // Headers automatically generated with multipart/form-data boundary
      });

      const data = await res.json();
      if (res.ok && data.success) {
        toast.success(data.message || "New client added successfully!");
        ...
        toast.error(data.message || "Failed to client creation.");
      }
    } catch (error) {
      console.error(error);
  ...
  const getHealthStyle = (health) => {
    switch (health) {
      case "Excellent":
        return "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border-emerald-200/40";
      case "Good":
        return "bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 border-blue-200/40";
      case "At Risk":
        return "bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400 border-rose-200/40";
      default:
      ...
      try {
        ...
        if (response.ok && result.success) {
          ...
          const mappedClients = result.data.map((client) => ({
            ...
            liaison: client.accountDirector,
            email: client.liaisonEmail,
            industry: client.industry,
            revenue: `$${client.pipelineBudget}`,
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
              Manage institutional partners, account health metrics, and fiscal
              pipeline revenue.
            </p>
          </div>
                ...
                {isLoading
                  ...
                  : clients.map((client) => (
                      ...
                      >
                        <td className="py-4 px-6 flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-xl bg-gradient-to-br ${client.logoColor} flex items-center justify-center text-white font-black text-xs uppercase shadow-sm group-hover:scale-105 transition-transform`}
                          >
                        ...
                        <td className="py-4 px-6">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getHealthStyle(client.health)}`}
                          >
                            {client.health}
                          </span>
                        </td>
        ...
        <div className="block md:hidden space-y-4">
          {isLoading
            ? Array.from({ length: 3 }).map((_, idx) => (
                <div
                  key={idx}
                  className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 animate-pulse"
                >
            ...
            : clients.map((client) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-9 h-9 rounded-xl bg-gradient-to-br ${client.logoColor} flex items-center justify-center text-white font-black text-xs uppercase shrink-0`}
                      >
                        {client.name.substring(0, 2)}
                      </div>
                      <div>
                        <span className="font-bold text-slate-800 dark:text-slate-100 block text-base leading-tight">
                          {client.name}
                        </span>
                        <span className="text-xs text-slate-400 dark:text-slate-500 font-mono">
                  ...
                  <div className="bg-slate-50/50 dark:bg-slate-950/40 p-3 rounded-xl space-y-1 text-xs">
                    <span className="text-slate-400 dark:text-slate-500 block font-medium uppercase tracking-wider text-xxs">
                      Account Director
                    </span>
                    <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                      <span className="font-semibold">{client.liaison}</span>
                      <span className="text-slate-400 font-mono select-all text-xxs max-w-[150px] truncate">
                        {client.email}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs pt-1">
                    <div>
                      ...
                      <span className="text-slate-900 dark:text-slate-50 font-extrabold text-sm">
                        {client.revenue}
                      </span>
                    </div>
                    <div className="text-right">
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full font-semibold border ${getHealthStyle(client.health)}`}
                      >
                        {client.health}
                      </span>
                    </div>
        ...
        {!isLoading && clients.length === 0 && (
          <div className="text-center py-16 text-slate-400 dark:text-slate-500 space-y-2">
            <svg
              className="w-8 h-8 mx-auto stroke-current opacity-60"
              fill="none"
              viewBox="0 0 24 24"
            >
              <path
            ...
            <p className="font-medium text-sm">
              No onboarded client accounts cataloged.
            </p>
          </div>
        )}
      </div>
