      ...
      try {
        ...
        if (Array.isArray(rawUsers)) {
          const normalized = rawUsers.map((u) => ({
            ...
            avatarUrl: u.avatarUrl || u.avatar || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150"
          }));
          setUsers(normalized);
        }
      } catch (error) {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Manage user identities, access roles, and account statuses.</p>
          </div>
                ...
                {isLoading ? (
                  Array.from({ length: 3 }).map((_, idx) => (
                    <tr key={idx} className="animate-pulse">
                      ...
                      <td className="py-4 px-6"><div className="h-8 w-16 ml-auto bg-slate-200 dark:bg-slate-800 rounded-lg" /></td>
                    </tr>
                  ))
                ) : (
                  users.map((user) => (
                    <tr key={user._id} className="group hover:bg-slate-50/60 dark:hover:bg-slate-800/20 transition-all duration-200">
                      ...
                      <td className="py-4 px-6">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${user.active ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${user.active ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`} />
                          {user.active ? "Active" : "Disabled"}
                        </span>
                      </td>
                      <td className="py-4 px-6 text-slate-500 dark:text-slate-400 text-xs">{formatDate(user.createdAt)}</td>
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handleOpenEditModal(user)} className="p-2 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-all active:scale-90">
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <img src={user.avatarUrl} alt={user.name} className="w-10 h-10 rounded-full object-cover ring-2 ring-slate-100 dark:ring-slate-800" />
                  <div>
              ...
              <div className="flex items-center justify-between text-xs border-t border-slate-100 dark:border-slate-800/60 pt-3">
                <div>
                  <span className="text-slate-400 dark:text-slate-500 block mb-1 font-medium">Role</span>
                ...
                <div className="text-right">
                  ...
                  <span className={`inline-flex items-center gap-1.5 font-medium ${user.active ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden transform transition-all">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-lg font-bold">{editingUser ? "Edit User Profile" : "Create New User"}</h3>
